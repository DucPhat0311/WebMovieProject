	...
	<div class="container">
		<div class="auth-box">
			...
			<form action="${pageContext.request.contextPath}/register" method="post">
				<div class="form-row">
					<div class="form-group">
						...
						<div class="input-with-icon">
                            ...
                            <input type="text" id="fullName" name="fullName" placeholder="Nhập họ tên đầy đủ" required value="${param.fullName}" minlength="2" maxlength="50">
						</div>
						<c:if test="${not empty fullNameError}">
							<div class="field-error"><i class="fas fa-exclamation-circle"></i> ${fullNameError}</div>
						</c:if>
					</div>
					...
					<div class="form-group">
						...
						<c:if test="${not empty emailError}">
							<div class="field-error"><i class="fas fa-exclamation-circle"></i> ${emailError}</div>
						</c:if>
					</div>
				</div>
				...
				<div class="form-row">
					<div class="form-group">
						...
						<c:if test="${not empty passwordError}">
							<div class="field-error"><i class="fas fa-exclamation-circle"></i> ${passwordError}</div>
						</c:if>
					</div>
					...
					<div class="form-group">
						...
						<div class="input-with-icon">
                            ...
                            <i class="fas fa-eye toggle-password" onclick="togglePwd('confirmPassword', this)"></i>
						</div>
						<c:if test="${not empty confirmPasswordError}">
							<div class="field-error"><i class="fas fa-exclamation-circle"></i> ${confirmPasswordError}</div>
						</c:if>
					</div>
				</div>
				...
				<div class="form-row">
					<div class="form-group">
						...
						<c:if test="${not empty phoneError}">
							<div class="field-error"><i class="fas fa-exclamation-circle"></i> ${phoneError}</div>
						</c:if>
					</div>
				...
				<div class="terms">
					<label> 
                        <input type="checkbox" name="agreeTerms" value="true" required ${param.agreeTerms=='true' ? 'checked' : ''}>
						Tôi đồng ý với <a href="${pageContext.request.contextPath}/terms">Điều khoản</a> và <a href="${pageContext.request.contextPath}/privacy">Chính sách bảo mật</a> <span class="required">*</span>
					</label>
					<c:if test="${not empty termsError}">
						<div class="field-error"><i class="fas fa-exclamation-circle"></i> ${termsError}</div>
					</c:if>
				</div>
